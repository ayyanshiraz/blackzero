...
export default function WhatsAppChat() {
  ...
  }, [messages, currentMenu, selectedServiceIndex, userSelection, clientName, clientNationality, isLoaded]);
  ...
  const handleSend = async () => {
    ...
    if (currentMenu === "live_chat" && userMsg.toLowerCase() !== "menu") {
      try {
        await fetch("http://localhost:5000/api/send-message", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ message: `💬 Client (${clientName || "Guest"}): ${userMsg}` })
        });
      } catch (err) {
    ...
    setTimeout(async () => {
        ...
        if (userMsg === "1") {
          ...
          try {
            await fetch("http://localhost:5000/api/send-message", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ message: `⚠️ User clicked Speak to an Agent now from post-submission menu. They are now in live chat mode.` })
            });
          } catch (err) {
            console.error(err);
          }
      ...
      if (currentMenu === "ask_nationality") {
        setClientNationality(userMsg);
      ...
      if (currentMenu === "ask_name") {
        ...
        setCurrentMenu("ask_nationality");
        setMessages(prev => [
          ...prev,
          { sender: "system", text: `🌍 What is your nationality?\n\n(e.g. Pakistani, British, American...)` }
        ]);
        return;
      }
    ...
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      {isOpen ? (
        ...
        <div className="flex flex-col items-center gap-4 relative">
          ...
          {/* Top Icon: WhatsApp (Using your exact SVG) */}
          <button
            onClick={() => setIsOpen(true)}
            aria-label="Chat on WhatsApp"
            className="relative flex items-center justify-center w-14 h-14 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg hover:shadow-2xl transition-all duration-300 ease-out hover:scale-110 z-10"
          >
            <svg
          ...
          {/* Bottom Icon: Chatbot / Black Zero Logo */}
          <div className="relative flex items-center justify-center">
            
            {/* Notification Pop-up Tooltip - Aligned with Chatbot Icon */}
            <div 
            ...
            >
              {/* Note: Ensure /logo.png is the correct path for your image */}
